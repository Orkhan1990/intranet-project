...
export class FistMigration1733917078906 implements MigrationInterface {
    name = 'FistMigration1733917078906'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`order_history\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`userId\` int NULL, \`orderId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`orders\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`project\` varchar(255) NULL, \`card_number\` varchar(255) NULL, \`order_type\` varchar(255) NOT NULL, \`manufacturer\` varchar(255) NOT NULL, \`model\` varchar(255) NOT NULL, \`chassis_number\` varchar(255) NOT NULL, \`engine_number\` varchar(255) NOT NULL, \`produce_year\` varchar(255) NOT NULL, \`km\` varchar(255) NOT NULL, \`vehicle_number\` varchar(255) NOT NULL, \`payment_type\` varchar(255) NOT NULL, \`delivering\` varchar(255) NOT NULL, \`delivering_type\` varchar(255) NOT NULL, \`initial_payment\` varchar(255) NOT NULL, \`status\` enum ('open', 'close') NOT NULL DEFAULT 'open', \`stage\` enum ('created', 'invastigation', 'calculation', 'finish') NOT NULL DEFAULT 'created', \`comment\` varchar(255) NOT NULL, \`oil\` tinyint NOT NULL DEFAULT 0, \`userId\` int NULL, \`clientId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
