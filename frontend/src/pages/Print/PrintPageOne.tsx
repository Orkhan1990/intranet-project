...
const PrintPageSecond = () => {
  ...
  const totalPrice = cardDetails?.cardJobs?.reduce(
    (acc: number, job: any) => acc + (Number(job.price)*(1 - (job.discount ?? 0) / 100)),
    0
  );
  ...
  return (
    <div className="bg-white text-black  print:p-2 max-w-full text-sm print:text-xs">
          ...
          <tbody>
            {cardDetails?.cardJobs?.map((job: any, i: number) => (
              <tr key={i} className="border-b border-gray-300">
                ...
                <td className="px-4 py-2 text-right whitespace-nowrap">
                  {formatPrice(+(job.price)*(1 - (job.discount ?? 0) / 100))}
                </td>
              </tr>
            ))}
            <tr>
     ...
     <div className="flex flex-col gap-5 pl-5 mt-10">
      <p className="font-semibold text-sm ">Avtomobilin kabinasında qiymətli, zərərli və kabinanı qaldırarkən xələl gətirə biləcək heç nə yoxdur.<br/> Bunu öz imzamla təsdiqləyirəm.</p>
      <p className="font-semibold">İmza:______________________</p>
     </div>
