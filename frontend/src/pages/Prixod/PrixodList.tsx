...
export const PrixodList = () => {
  ...
  const {
    ...
    error: ordersError,
  } = useSelector((state: RootState) => state.prixod);
  ...
  return (
    <div className="min-h-screen mt-[100px] mb-[100px]">
      ...
      <table className="ml-5  mt-10 border-collapse border border-gray-300 ">
        ...
        <tbody>
          {prixods
    .filter((prixod) => prixod.confirm === false) // 👈 only show unconfirmed
    .map((prixod) => (
            <tr
              key={prixod.id}
            ...
            >
              ...
              <td className="w-64 text-center ">{prixod.order?.id}</td>
