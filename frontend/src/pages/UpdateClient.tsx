...
const UpdateClient = () => {
  ...
  useEffect(()=>{
    ...
    const getClientData=async()=>{
         try {
          const res = await fetch(`http://localhost:3013/api/v1/client/getClient/${id}`, {
            method: "GET",
  ...
  const onSubmit = async (values:ClientInterface) => {
    try {
      const res = await fetch(`http://localhost:3013/api/v1/client/updateClient/${id}`, {
        method: "POST",
        credentials: "include", // added this part
        headers: {
          "Content-Type": "application/json",
      ...
      <div className="m-10 max-w-2xl mx-auto">
        ...
        <div>
          ...
          <div>
            ...
            <Field as={TextInput} id="voen" type="text" placeholder="Vöen" required name="voen"/>
          </div>
